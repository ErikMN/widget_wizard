...
test_file_upload_replaces_existing_file_on_finish(void **state)
{
  ...
  assert_true(file_upload_begin(&test_state->upload,
                                test_state->file.filename,
                                strlen(test_state->file.filename),
                                sizeof(uploaded_payload) - 1,
                                &result));
  ...
  test_support_assert_file_contents(test_state->file.path, uploaded_payload, sizeof(uploaded_payload) - 1);
}
...
main(void)
{
  const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test_setup_teardown(
        test_file_upload_replaces_existing_file_on_finish, setup_file_upload_test, teardown_file_upload_test),
    cmocka_unit_test_setup_teardown(
