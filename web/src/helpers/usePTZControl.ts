 ...
 * usePTZControl
 *
 * Custom React hook for PTZ (Pan-Tilt-Zoom) camera control via keyboard and programmatic commands.
 *
 * Keyboard controls:
 ...
 * - Key repeat prevention (continuous movement from single keypress)
 * - Combined arrow key movements (e.g., up+right for diagonal)
 * - Window blur detection (stops movement when focus is lost)
 * - Input element detection (ignores keys when typing in forms)
 * - Cleanup on unmount (ensures camera stops)
 */
...
interface UsePTZControlOptions {
  /** Current camera channel number */
  currentChannel: string;
  /** Enable keyboard control (default: true) */
 ...
 * @returns Control functions for programmatic PTZ control
 */
export const usePTZControl = ({
  currentChannel,
  enableKeyboard = true
}: UsePTZControlOptions) => {
  ...
  /* Return control functions for programmatic use */
  return {
